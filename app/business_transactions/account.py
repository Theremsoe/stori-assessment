...
from app.business_transactions.entities import AccountSummary, Transaction, TransactionRaw

def account_summary_from_file(filepath: str) -> AccountSummary:
    """
    Given a file as input generates an account summary information

    Definitions:
        txn: transaction
    """
    with open(filepath, 'r', encoding='utf-8') as file_stream:
        reader = DictReader(file_stream, strict=True)
        return account_summary(reader)
